...
async function guardarCelda(fecha, empId) {
  ...
  const fechaStr = String(fecha).slice(0, 10); // asegurar formato YYYY-MM-DD
  ...
  try {
    ...
    if (existente) {
      ...
      state.planificacion.push({
        ...nuevo,
        fecha: fechaStr,
        empleado_id: parseInt(nuevo.empleado_id, 10),
        horas: parseFloat(nuevo.horas)
      });
    }
    cerrarModal(); render(); toast('Guardado', 'success');
  } catch (e) { toast('Error: ' + e.message, 'error'); }
